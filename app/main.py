...
app = FastAPI(root_path="/invoice-scanner")
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(
        ...
        content=format_error_response(
            error_message=str(exc.detail), error_type="HTTPException"
        ),
    )
...
async def general_exception_handler(request: Request, exc: Exception):
    return JSONResponse(
        status_code=500,
        content=format_error_response(error_message=str(exc), error_type="Exception"),
    )
